...
StructureSpawn.prototype.buildCreep =
    ...
    let n = {1: 2, 2: 4, 3:5, 4: 6, 5:7, 6: 8, 7: 10, 8: 12};
    let maxParts = n[rcl.toString()];
    lg(maxParts);
    ...
    switch (roleName) {
      case 'hauler':
        ...
        numberOfParts.carry = Math.min(Math.floor(this.room.energyAvailable/100), maxParts);
        numberOfParts.move =Math.min(Math.floor(this.room.energyAvailable/100), maxParts);
        break;
      case 'miner':
        newName = 'Miner' + Game.time;
        numberOfParts.work = Math.min(Math.floor((this.room.energyAvailable -50)/100), maxParts);
      ...
      default:
        ...
        numberOfParts.work = Math.min(Math.floor(this.room.energyAvailable/200), maxParts);
        numberOfParts.carry = Math.min(Math.floor(this.room.energyAvailable/200), maxParts);
        numberOfParts.move = Math.min(Math.floor(this.room.energyAvailable/200), maxParts);
    }
...
//
// 		// CREATE CREEP WITH THE CREATED BODY AND THE GIVEN ROLE
// 		return this.spawnCreep(body, newName, {memory: {role: 'miner', full: false, energyOnContainer: true, home: this.room.name}});
// 	};
